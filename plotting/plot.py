...
def plot_results(migrations, costs, fes, actions, rewards, axs):
    ...
    axs[0,0].plot(migrations, costs)
    axs[0,0].set_xlabel('Migration')
    axs[0,0].set_ylabel('Cost')

    if(len(migrations) > 3000):
        axs[1,1].plot(np.convolve(migrations[3000:], np.ones((100,))/100, mode='valid'), np.convolve(costs[3000:], np.ones((100,))/100, mode='valid'))
        axs[1,1].set_xlabel('Migration')
        axs[1,1].set_ylabel('Cost')

    
    axs[0,1].plot(np.convolve(migrations, np.ones((100,))/100, mode='valid'), np.convolve(actions[0], np.ones((100,))/100, mode='valid'))
    axs[0,1].set_xlabel('Migration')
    axs[0,1].set_ylabel('Move')
    
    axs[1,0].plot(np.convolve(migrations, np.ones((100,))/100, mode='valid'), np.convolve(actions[1], np.ones((100,))/100, mode='valid'))
    axs[1,0].set_xlabel('Migration')
    axs[1,0].set_ylabel('Step')

    axs[2,0].plot(np.convolve(migrations, np.ones((100,))/100, mode='valid'), np.convolve(rewards, np.ones((100,))/100, mode='valid'))
    axs[2,0].set_xlabel('Migration')
    axs[2,0].set_ylabel('Reward')
        
    axs[2,1].clear()
    axs[2,1].text(0.15,0.5, str(-500 - costs[-1])+ " / " + str(costs[-1]))
    axs[2,1].text(0.25,0.25, migrations[-1])
