...
class CustomEnv(gym.Env):
    ...
    def __init__(self, 
     PopSize, dimension, CostFunctions,
      Max_FEs, HistoryLen, f_global, x_global, function_name, write, pretrained, log_dir_results, m, n, k
      ):
        super().__init__()
        ...
        self.observation_space = Dict({
            ...
            "action1": Box(shape=(self.HistoryLen,1), low=-float('inf'), high=float('inf')),
            "action2": Box(shape=(self.HistoryLen,1), low=-float('inf'), high=float('inf')),
            "action3": Box(shape=(self.HistoryLen,1), low=-float('inf'), high=float('inf')),
            })
...
# k=15 m=10 n=5
        self.kFunc = lambda x : 1 + round(self.PopSize* (0.31 + 0.25 * x[0])) if 1 + round(self.PopSize* (0.31 + 0.25* x[0])) < self.PopSize else self.PopSize
        self.nFunc = lambda x : 1 + round(self.PopSize* (0.31 + 0.25 * x[2]) *(0.31 + 0.25 * x[1]))  if 1 + round(self.PopSize* (0.31 + 0.25* x[2]) *(0.31 + 0.25* x[1]))  < self.PopSize * (0.31 + 0.25* x[2]) else round(self.PopSize * (0.31 + 0.25* x[2]))
        self.mFunc = lambda x : 1 + round(self.PopSize* (0.31 + 0.25 * x[2])) if 1 + round(self.PopSize* (0.31 + 0.25* x[2])) < self.PopSize else self.PopSize
    ...
    def migrate_to_leader(self, Migrant, Leader, move, action):
        ...
        PRT = 0.05 + 0.9*(self.FEs / self.Max_FEs)
    ...
    def self_organizing(self, action):
        ...
        # Migrant selection: m
        M = np.random.choice(range(self.PopSize),self.mFunc(action),replace=False)
        M_sort = np.argsort(self.fitness[M])
        for j in range(self.nFunc(action)):
            # Get the Migrant position (solution values) in the current population
            Migrant = self.pop[:, M[M_sort[j]]].reshape(self.dimension, 1)

            # Leader selection: k
            K = np.random.choice(range(self.PopSize),self.kFunc(action),replace=False)
    ...
    def step(self, action):
        ...
        self.action1.append(action[0])
        self.action2.append(action[1])
        self.action3.append(action[2])
        info = {}
        ...
        if self.write:
            with open(os.path.join(self.log_dir_results,"action_" + "".join([c for c in self.function_name if re.match(r'\w', c)]) + ".txt"), "a") as f:
                f.write(str(self.total_migrations) + "\t" + str(self.mFunc(action))  + "\t" + str(self.kFunc(action))  + "\t" + str(self.nFunc(action))  + "\t" + str(self.the_best_cost) + "\t" + str(self.FEs) + "\t" + str(reward) + "\n")
    ...
    def get_observation(self):     
        ...
        action1 = np.array(self.action1, dtype=np.float32)
        action2 = np.array(self.action2, dtype=np.float32)
        action3 = np.array(self.action3, dtype=np.float32)
        pad_value = self.history[0]  
        pad_difference_history = self.difference_history[0]  
        pad_action1 = self.action1[0]  
        pad_action2 = self.action2[0]  
        pad_action3 = self.action3[0]  
        ...
        padded_action1 = np.pad(action1, (self.HistoryLen - action1.shape[0],0), mode='constant', constant_values=pad_action1).reshape(self.HistoryLen,1)
        padded_action2 = np.pad(action2, (self.HistoryLen - action2.shape[0],0), mode='constant', constant_values=pad_action2).reshape(self.HistoryLen,1)
        padded_action3 = np.pad(action3, (self.HistoryLen - action3.shape[0],0), mode='constant', constant_values=pad_action3).reshape(self.HistoryLen,1)
        return {
            "history": padded_history,
            "difference_history": padded_difference_history,
            "action1": padded_action1,
            "action2": padded_action2,
            "action3": padded_action3,
        }
